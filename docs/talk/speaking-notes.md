...
| 15–30 | Cursor: rules, one skill, schema Markdown. Official skills-for-fabric. | `.cursor/rules/`, skill, `schema/`, `npm run skills:install` |
...
1. Demo DW loaded (`npm run warehouse:sql -- --demo` + run SQL)
2. Semantic model from warehouse **Reporting** tab
3. Fabric App item + `.env.fabric` with publishable key
...
1. `npm run skills:install` — or `npx skills add microsoft/skills-for-fabric -a cursor -g -y`
